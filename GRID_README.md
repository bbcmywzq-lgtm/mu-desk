...
Grid（原栖格，工程名 DesktopOrganizer）是 MU Desk 内本地优先的 Windows 桌面整理工具。它把真实桌面文件映射到可移动、可缩放、可折叠的毛玻璃分区中，让桌面各得其所。普通分组整理只修改本机元数据，不会移动、重命名或删除原文件。

Grid 是 MU Desk 的内部模块之一。总包入口见 [README.md](README.md)，工具箱的完整模块说明见 [TOOLBOX_README.md](TOOLBOX_README.md)。改名不改变原有数据路径和备份格式。
